...
public class MongoDB implements InterfaceBaseDeDatos {
    ...
    private Document mapearProductoADocumento(Producto producto) {
        ...
        switch (producto.getProductoTipo().toLowerCase()) {
            case "arbol":
                doc.append("altura", ((Arbol) producto).getArbolAltura());
                break;
            case "flor":
                doc.append("color", ((Flor) producto).getFlorColor());
                break;
            case "decoracion":
                doc.append("material", ((Decoracion) producto).getDecoracionMaterial());
                break;
        }
        return doc;
    }
